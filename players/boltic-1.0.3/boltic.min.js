    ...
    e.prototype = {
        init: function() {
            ...
            var w = e.continuous ? "boltic-paused boltic-continuous " : "boltic-stopped ",
                x = "";
            	x += '<div id="boltic" class="boltic-recss boltic-' + e.theme + " boltic-" + e.ui + " " + h + " " + i + " " + j + " boltic-" + e.startTemplate + " boltic-" + e.position + " " + l + " " + m + n + " " + p + " " + r + " " + q + '">',
            	
            	x += '<div id="boltic-wrapper">', 
            	x += s, 
            	x += '<div id="boltic-top" class="boltic-default-art">', 
            	x += '<a href="http://app.mastercast.com.br/player/site?v='+window.btoa(window.btoa(e.port))+'" target="_blank"><div class="boltic-album-wrapper"><i class="boltic-mdi boltic-mdi-album"></i></div></a>', 
            	x += '<iframe style="width:0;height:0;border:none" src="https://app-ssl.mastercast.com.br/player/tracker?v='+md5(e.port).substr(0,12)+'&s='+window.btoa(window.location.href)+'"></iframe>',            	
            	x += '<div id="boltic-top-content" class="boltic-top-content">', 
            	x += '<div id="boltic-nowplaying"><p id="boltic-np-title"></p><p id="boltic-np-artist"></p></div>', 
            	x += '<div id="boltic-linear"></div>', 
            	x += "</div>", 
            	x += '<div id="boltic-play" class="' + w + '">', 
            	x += '<i class="boltic-mdi boltic-mdi-play"></i>', 
            	x += '<i class="boltic-mdi boltic-mdi-pause"></i>', 
            	x += '<i class="boltic-mdi boltic-mdi-stop"></i>', 
            	x += "</div></div>" + t + "</div></div>", 
            	f.insertAdjacentHTML("beforeend", x), e.onReady(), (e.statusBar || e.currentTrack || e.currentArtist) && this.stats(function(a) {
                var b = e.serverInfo;
                if (b && e.statusBar) {
                ...
                s = function(d) {
                    ...
                    g.currentTrack && (i.textContent = s), g.currentArtist && (j.textContent = t), g.artwork && e.artwork(t, s, function(d) {
                        if (d) {
                            ...
                            e = 'https://app-ssl.mastercast.com.br/uploads/'+Cookies.get('mcPorta')+'/boltic.jpg?t='+(new Date()).getTime(); 
...
function playerBoltic(d){	
	...
	$.boltic({			
		...
		played: true, 	
		artwork: ['https://app-ssl.mastercast.com.br/uploads/'+d.port+'/boltic.jpg?t='+(new Date()).getTime()], 
		position: d.position, 
		theme: d.theme,
